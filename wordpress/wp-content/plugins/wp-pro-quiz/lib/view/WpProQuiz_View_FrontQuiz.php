...
class WpProQuiz_View_FrontQuiz extends WpProQuiz_View_View
{
    ...
    private function loadButtonNames()
    {
        ...
        $names = array(
            'start_quiz' => __('Bắt đầu thi', 'wp-pro-quiz'),
            'restart_quiz' => __('Thi lại', 'wp-pro-quiz'),
            'quiz_summary' => __('Tóm tắt bài thi', 'wp-pro-quiz'),
            'finish_quiz' => __('Nộp bài', 'wp-pro-quiz'),
            'quiz_is_loading' => __('Bài thi đang được lựa chọn. Bạn chờ tí nhé...', 'wp-pro-quiz'),
            'lock_box_msg' => __('Bạn đã hết lượt để thi. Xin vui lòng nạp thêm tiền để thi tiếp.',
                'wp-pro-quiz'),
            //'only_registered_user_msg' => __('You must sign in or sign up to start the quiz.', 'wp-pro-quiz'),
            'only_registered_user_msg' => __('Bạn cần đăng nhập hoặc đăng ký để bắt đầu thi.', 'wp-pro-quiz'),
            'prerequisite_msg' => __('Bạn phải hoàn thành bài thi sau để có thể làm bài thi này:', 'wp-pro-quiz')
        );
    ...
    private function showTimeLimitBox()
    {
        ...
        <div style="display: none;" class="wpProQuiz_time_limit">
            <div class="time"><?php _e('Thời gian còn lại', 'wp-pro-quiz'); ?>: <span>0</span></div>
            ...
            <div class="wpProQuiz_reviewLegend">
                <ol>
                    <li>
                        ...
                        <span class="wpProQuiz_reviewText"><?php _e('Đã trả lời', 'wp-pro-quiz'); ?></span>
                    </li>
<!--                    <li>
    ...
    private function showResultBox($result, $questionCount)
    {
        ...
        <div style="display: none;" class="wpProQuiz_results">
            <h4 class="wpProQuiz_header"><?php _e('Kết quả', 'wp-pro-quiz'); ?></h4>
            <?php if (!$this->quiz->isHideResultCorrectQuestion()) { ?>
                <p>
                    <?php printf(__('Bạn trả lời đúng %s trên %s câu hỏi', 'wp-pro-quiz'),
                        '<span class="wpProQuiz_correct_answer">0</span>', '<span>' . $questionCount . '</span>'); ?>
                </p>
            <?php }
            if (!$this->quiz->isHideResultQuizTime()) { ?>
                <p class="wpProQuiz_quiz_time">
                    <?php _e('Thời gian làm bài: <span></span>', 'wp-pro-quiz'); ?>
                </p>
            <?php } ?>
            ...
            <?php if (!$this->quiz->isHideResultPoints()) { ?>
                <p class="wpProQuiz_points">
                    <?php printf(__('Bạn đạt được %s trên %s điểm (%s)', 'wp-pro-quiz'), '<span>0</span>',
            ...
            <div style="margin: 10px 0px;">
                ...
                if (!$this->quiz->isBtnViewQuestionHidden()) { ?>
                    <input class="wpProQuiz_button" type="button" name="reShowQuestion"
                           value="<?php _e('Xem đáp án', 'wp-pro-quiz'); ?>">
                <?php } ?>
                <?php if ($this->quiz->isToplistActivated() && $this->quiz->getToplistDataShowIn() == WpProQuiz_Model_Quiz::QUIZ_TOPLIST_SHOW_IN_BUTTON) { ?>
                    <input class="wpProQuiz_button" type="button" name="showToplist"
                           value="<?php _e('Xếp hạng', 'wp-pro-quiz'); ?>">
                <?php } ?>
            </div>
        </div>
        <?php
                ...
                foreach ($this->question as $question) {
                    ...
                    <li class="wpProQuiz_listItem" style="display: none;">
                        ...
                        <h5 style="<?php echo $this->quiz->isHideQuestionNumbering() ? 'display: none;' : 'display: inline-block;' ?>"
                            ...
                            <span><?php echo $index; ?></span>. <?php _e('Câu hỏi', 'wp-pro-quiz'); ?>
                        </h5>
